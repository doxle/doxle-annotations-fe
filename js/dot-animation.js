(function () {
  ...
  function chooseGrid(container) {
    ...
    // Reduce dots for better performance but keep the effect
    let targetDots = 800; // Reduced from 2000
    if (window.devicePixelRatio > 2) targetDots = Math.min(targetDots, 600);
    if (matchMedia && matchMedia('(pointer:coarse)').matches) targetDots = Math.min(targetDots, 500);
    if (matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches) targetDots = Math.min(targetDots, 300);
