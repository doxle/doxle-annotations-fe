...
pub fn ImportBlockPage(props: ImportBlockPageProps) -> Element {
    ...
    let handle_submit = move |evt: Event<FormData>| {
        ...
        if *is_uploading.read() {
            crate::core::progress::show_info("Upload in progress...");
            return;
        }
        ...
        let file = match selected_file.read().clone() {
            Some(f) => f,
            None => {
                crate::core::progress::show_error("Please select a zip file to import");
                return;
            }
        };
            ...
            .await
            {
                Ok(result) => {
                    ...
                    .await
                    {
                        Ok(s) => s,
                        Err(e) => {
                            crate::core::progress::show_error_persistent(&format!("Failed to start import: {}", e));
                    ...
                    for _ in 0..2400 {
                        if flag.get() {
                            crate::core::progress::show_info("Stopped polling import status (job continues in background)");
                            done = true;
                            break;
                        }
                        ...
                        .await
                        {
                            Ok(s) => s,
                            Err(e) => {
                                crate::core::progress::show_error_persistent(&format!("Import status failed: {}", e));
                                done = true;
                                break;
                            }
                        };
                        ...
                        if status.status == "completed" {
                            crate::core::progress::show_success_for(
                                &format!(
                        ...
                        if status.status == "failed" {
                            ...
                            crate::core::progress::show_error_persistent(&msg);
                            done = true;
                            break;
                        }
                    }
                    ...
                    if !done {
                        crate::core::progress::show_error_persistent("Import timed out while waiting for completion");
                    }
                }
                Err(e) if e == "cancelled" => {
                    crate::core::progress::show_info("Upload cancelled");
                }
                Err(e) => {
                    dioxus::logger::tracing::error!("❌ Import upload failed: {}", e);
                    crate::core::progress::show_error_persistent(&format!("Import failed: {}", e));
                }
            }
            is_uploading.set(false);
        });
                                                ...
                                                if let Ok(input) = target.dyn_into::<web_sys::HtmlInputElement>() {
                                                    if let Some(files) = input.files() {
                                                        if let Some(file) = files.get(0) {
                                                            let size = file.size();
                                                            crate::core::progress::show_info_for(
