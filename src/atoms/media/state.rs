use super::api::{self, BlockImage};
use dioxus::prelude::*;

// Global signals for images state
pub static IMAGES: GlobalSignal<Vec<BlockImage>> = Signal::global(|| Vec::new());
...
pub async fn load_block_images(project_id: &str, block_id: &str) {
    ...
    *IMAGES_ERROR.write() = None;
    ...
    match api::list_block_images(project_id, block_id).await {
        Ok(images_list) => {
...
pub fn get_current_image() -> Option<BlockImage> {
