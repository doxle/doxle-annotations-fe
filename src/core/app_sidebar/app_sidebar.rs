...
use crate::blocks::api::{BlockLabel, api_update_label_color};
        ...
        div {
            ...
            if active_tab() == SidebarTab::Labels {
                div {
                            ...
                            div {
                                ...
                                onclick: move |_| {
                                    ...
                                    if let Some(mut tool_sig) = selected_tool {
                                        if let Some(label) = crate::blocks::state::LABELS.read().iter().find(|l| l.label_id == lid_select) {
                                            ...
                                            onchange: move |e| {
                                                ...
                                                spawn(async move {
                                                    match api_update_label_color(&bid, &lid, new_color.clone()).await {
                                                        Ok(updated) => {
                                                            crate::blocks::state::LABELS.write().iter_mut().for_each(|l| {
                                                                if l.label_id == lid {
                                ...
                                {{
                                    let bbox_count = annotations.iter().filter(|a| a.label_id == lid && matches!(a.geometry, crate::core::svg_canvas::Geometry::BBox { .. })).count();
                                    let poly_count = annotations.iter().filter(|a| a.label_id == lid && matches!(a.geometry, crate::core::svg_canvas::Geometry::Polygon { .. })).count();
                   ...
                   // Total row
                   {{
                       let total_bbox = annotations.iter().filter(|a| matches!(a.geometry, crate::core::svg_canvas::Geometry::BBox { .. })).count();
                       let total_poly = annotations.iter().filter(|a| matches!(a.geometry, crate::core::svg_canvas::Geometry::Polygon { .. })).count();
                       rsx! {
                           div {
