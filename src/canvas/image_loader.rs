...
use web_sys::{Blob, Request, RequestCredentials, RequestInit, Response, Url};
...
pub async fn load_authenticated_image(url: &str) -> Result<String, String> {
    ...
    // Create request with credentials (no cache-busting for CloudFront caching)
    ...
    let response: Response = resp_value.dyn_into().map_err(|_| "Invalid response type")?;
    ...
    let blob: Blob = blob_value.dyn_into().map_err(|_| "Invalid blob type")?;
