...
pub struct IWallsData {
    pub page_id: String,
    pub segments: Vec<Segment>,
}

#[derive(Debug, Clone)]
pub struct IWallsClickResult {
...
pub fn total_iwalls_lm_m(iwalls: &HashMap<String, IWallsData>, scales: &HashMap<String, ScaleData>) -> f64 {
    iwalls
        .iter()
        .map(|(page_id, data)| {
            let total_px = data
...
pub fn compute_live_iwalls_lm_m(
    iwalls: &HashMap<String, IWallsData>,
    start_point: Option<(f64, f64)>,
    cursor_world: Option<(f64, f64)>,
    page_id: Option<&str>,
    scales: &HashMap<String, ScaleData>,
) -> f64 {
    let base = total_iwalls_lm_m(iwalls, scales);
...
pub fn IWallsPanelSection(
    ...
    rsx! {
        p {
            ...
            "Draw internal walls as separate broken line segments."
        }
        div {
